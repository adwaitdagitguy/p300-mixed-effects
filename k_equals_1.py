...
for dataset_name, cfg in dataset_configs.items():
    ...
    # Store stability records across grouping variables
    stability_records = []
    ...
    for group_var, meta_dict in cfg['metadata'].items():
        ...
        # Record stability and fit stats from gate_df.attrs
    ...
    if stability_records:
        ...
        df_stability.to_csv(f"k1_stability_summary_{dataset_name}.csv", index=False)
        print(f"\n{'='*80}")
        print(f"K=1 LME STABILITY & DIAGNOSTICS REPORT ({dataset_name.upper()})")
        print(f"{'='*80}")
        print(df_stability[[
            'group_var', 'n_attempted', 'n_converged', 'pct_converged',
            'n_failed', 'pct_failed', 'sigma2_median', 'sigma2_iqr',
            'n_singular', 'pct_singular', 'null_rej_gxbin'
        ]].to_string(index=False))
        print(f"\nDetailed stability table exported to: k1_stability_summary_{dataset_name}.csv")
