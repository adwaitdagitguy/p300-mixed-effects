...
def load_atpv_all_k(atpv_prefix, k_range):
    ...
    Extracts subject_id from blob paths if not explicitly a column.
    """
    frames = []
    blobs = [b.name for b in container.list_blobs(name_starts_with=atpv_prefix) if b.name.endswith('.parquet')]
    if not blobs:
        return None
    print(f"  Found {len(blobs)} total ATPV parquet blobs under '{atpv_prefix}'...")

    for b_path in blobs:
        try:
            # Parse k from path e.g. /k_15/
            k_val = None
            for part in b_path.split('/'):
                if part.startswith('k_') and part[2:].isdigit():
                    k_val = int(part[2:])
                    break
            
            # Parse subject from filename e.g. subject_01.parquet or s01.parquet
            fname = os.path.splitext(os.path.basename(b_path))[0]
            subj_digits = ''.join(filter(str.isdigit, fname))
            subj_id = subj_digits.zfill(2) if subj_digits else None

            df = download_blob_df(b_path)
            if df.empty:
                continue

            if 'k' not in df.columns and k_val is not None:
                df['k'] = k_val

            # Ensure subject_id column exists
            has_subj_col = any(c in df.columns for c in ['subject_id', 'subject', 'Subject'])
            if not has_subj_col and subj_id is not None:
...
for dataset_name, cfg in dataset_configs.items():
    ...
    for group_var, meta_dict in cfg['metadata'].items():
        ...
        if atpv_df is not None and not atpv_df.empty and 'subject_id' in atpv_df.columns:
